...
{  
    ...
    private_node_handle.param("max_ground_height_threshold", max_ground_height_threshold_, 0.5);
            ...
            for (size_t j = 0; j < rows; ++j) 
            {
                ...
                if (count > 0) 
                {
                    ...
                    if (cov[i][j] > covariance_threshold_ * covariance_threshold_) 
                    {
                        ...
                        if ((mean_z < height_of_hole_) || (mean_z > max_ground_height_threshold_)) 
                        {
...
VPointCloud::Ptr Remove_ground::GetFilteredWithNearestNeighborVariance(VPointCloud::Ptr raw_pcl_)
{
    ...
    double sum_z[pcl_size], sum_zz[pcl_size];
    size_t neighbor_number[pcl_size];
    ...
    if(pcl_size > 0)
    {
        for(size_t i = 0; i < pcl_size; i++)
        {
            ...
            if(neighbor_number[i] > min_num_neighbour_points_threshold_)
            {
                double mean_z = sum_z[i] / (neighbor_number[i]+1);
                double cov = sum_zz[i] / (neighbor_number[i]+1) - mean_z * mean_z;
                if (cov > covariance_threshold_ * covariance_threshold_)
                {
                    obstacle_flag[i] = 1;
                    p_1.intensity = cov;
                }
